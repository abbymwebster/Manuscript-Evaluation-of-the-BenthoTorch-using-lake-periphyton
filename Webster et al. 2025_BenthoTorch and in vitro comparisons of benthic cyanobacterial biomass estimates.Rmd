...
Supplementary
```{r}
lightmetadata<- metadata %>% filter(Lake != "Canandaigua")
...
```{r}
QCdata<-read.csv("Std checks on different modules 2023 for R.csv")

FieldQC<- QCdata %>% filter(StdType == "Field")
NoFieldQC<- QCdata %>% filter(StdType != "Field")

AvgQCdata<-read.csv("Avgs of std checks by date for R.csv")
AvgFieldQC<- AvgQCdata %>% filter(StdType == "Field")
AvgNoFieldQC<- AvgQCdata %>% filter(StdType != "Field")
```
